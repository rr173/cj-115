...
let GroundwaterService = class GroundwaterService {
    ...
    async registerSmartMeter(dto) {
        ...
        if (!well)
            throw new common_1.NotFoundException('机井不存在');
        if (well.meter)
            throw new common_1.BadRequestException('该机井已登记电表');
        const existingMeter = await this.prisma.smartMeter.findUnique({
            where: { meterNo: dto.meterNo },
        });
        if (existingMeter)
            throw new common_1.BadRequestException('该表号已被使用');
        ...
        const meter = await this.prisma.smartMeter.create({
            data: {
                ...
                status: enums_1.MeterStatus.NORMAL,
                remark: dto.remark,
            },
            include: {
                well: {
    ...
    async updateSmartMeter(meterId, dto) {
        const meter = await this.prisma.smartMeter.findUnique({ where: { id: meterId } });
        if (!meter)
            throw new common_1.NotFoundException('电表不存在');
        if (dto.meterNo) {
    ...
    async reportMeterReading(dto) {
        ...
        if (!meter)
            throw new common_1.NotFoundException('电表不存在');
        if (meter.status !== enums_1.MeterStatus.NORMAL) {
            throw new common_1.BadRequestException(`电表状态为[${meter.status}]，暂无法上报读数，请先处理异常`);
        }
        ...
        if (currentReading < previousReading) {
            const alert = await this.prisma.meterAbnormalAlert.create({
                data: {
                    meterId: meter.id,
                    wellId: meter.wellId,
                    type: enums_1.MeterAbnormalType.READING_REVERSED,
            ...
            await this.prisma.smartMeter.update({
                where: { id: meter.id },
                data: { status: enums_1.MeterStatus.ABNORMAL },
            });
        ...
        if (activeQuota && activeQuota.blocked) {
            throw new common_1.BadRequestException(`分区[${zone.name}]本灌溉季电量配额已用尽(${activeQuota.usedKwh.toFixed(2)}/${activeQuota.totalKwh}度),禁止继续抽水,请管理员扩配或换季清零`);
        }
        let quotaId = null;
        ...
        if (activeQuota) {
            ...
            if (consumption > remainingKwh + 0.001) {
                throw new common_1.BadRequestException(`分区电量配额剩余${remainingKwh.toFixed(2)}度,本次需${consumption.toFixed(2)}度,配额不足,请管理员扩配`);
            }
        ...
        if (quotaWarningAfter && activeQuota) {
            await this.createAlert(zone.id, enums_1.GroundwaterAlertType.REDLINE_WARNING, enums_1.GroundwaterAlertLevel.WARNING, `分区[${zone.name}]本灌溉季电量配额已使用${((activeQuota.usedKwh + consumption) / activeQuota.totalKwh * 100).toFixed(1)}%,接近${QUOTA_WARNING_RATIO * 100}%预警线`);
        }
        if (quotaBlockedAfter && activeQuota) {
            await this.createAlert(zone.id, enums_1.GroundwaterAlertType.REDLINE_BLOCKED, enums_1.GroundwaterAlertLevel.CRITICAL, `分区[${zone.name}]本灌溉季电量配额已用尽${activeQuota.totalKwh}度,后续机井计量将被拦截`);
        }
        return {
            accepted: true,
            abnormal: false,
    ...
    async listMeterReadings(wellId, meterId, dateFrom, dateTo) {
        const where = {};
        if (wellId)
            where.wellId = wellId;
        if (meterId)
        ...
        if (dateFrom || dateTo) {
            where.reportedAt = {};
            if (dateFrom)
                where.reportedAt.gte = (0, dayjs_1.default)(dateFrom).startOf('day').toDate();
            if (dateTo)
                where.reportedAt.lt = (0, dayjs_1.default)(dateTo).add(1, 'day').startOf('day').toDate();
        }
    ...
    async listMeterAbnormalAlerts(resolved) {
        const where = {};
        if (resolved !== undefined)
    ...
    async resolveMeterAbnormal(dto) {
        ...
        if (alert.isResolved)
            throw new common_1.BadRequestException('该告警已处理');
    ...
    async createElectricityQuota(dto) {
        const zone = await this.prisma.irrigationZone.findUnique({ where: { id: dto.zoneId } });
        if (!zone)
            throw new common_1.NotFoundException('灌溉分区不存在');
        const startDate = (0, dayjs_1.default)(dto.startDate).startOf('day').toDate();
        const endDate = (0, dayjs_1.default)(dto.endDate).endOf('day').toDate();
        ...
        if (overlap) {
            throw new common_1.BadRequestException(`该分区在[${dto.startDate},${dto.endDate}]时段内已有电量配额,不能重复创建`);
        }
    ...
    async updateElectricityQuota(quotaId, dto) {
        const quota = await this.prisma.electricityQuota.findUnique({ where: { id: quotaId } });
        if (!quota)
            throw new common_1.NotFoundException('电量配额不存在');
        if (dto.totalKwh !== undefined && dto.totalKwh < quota.usedKwh) {
            throw new common_1.BadRequestException(`新配额${dto.totalKwh}度不能小于已用电量${quota.usedKwh.toFixed(2)}度`);
        }
    ...
    async listElectricityQuotas(zoneId) {
        const where = {};
        if (zoneId)
            where.zoneId = zoneId;
        ...
        return quotas.map((q) => {
            ...
            let status;
            if (usageRatio >= 1 || q.blocked)
                status = enums_1.ElectricityQuotaStatus.EXHAUSTED;
            else if (usageRatio >= QUOTA_WARNING_RATIO)
                status = enums_1.ElectricityQuotaStatus.WARNING;
            else
                status = enums_1.ElectricityQuotaStatus.NORMAL;
            return {
                ...q,
    ...
    async getElectricityQuotaUsage(quotaId) {
    ...
    async getWellReconciliation(wellId, dateFrom, dateTo) {
        const well = await this.prisma.pumpingWell.findUnique({
            where: { id: wellId },
            include: { zone: { select: { id: true, code: true, name: true } } },
        });
        if (!well)
            throw new common_1.NotFoundException('机井不存在');
        const startTime = dateFrom ? (0, dayjs_1.default)(dateFrom).startOf('day').toDate() : (0, dayjs_1.default)().startOf('year').toDate();
        const endTime = dateTo ? (0, dayjs_1.default)(dateTo).endOf('day').toDate() : new Date();
        const extractionRecords = await this.prisma.groundwaterExtractionRecord.findMany({
    ...
    async getZoneReconciliation(zoneId, dateFrom, dateTo) {
