...
let AccountingService = class AccountingService {
    ...
    async reportUsage(dto) {
        ...
        if (app.status !== enums_1.ApplicationStatus.SCHEDULED) {
            throw new common_1.BadRequestException('只有已安排(SCHEDULED)状态的申请才能上报实际用水量');
        }
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            await tx.waterApplication.update({
                where: { id: dto.applicationId },
                data: { status: enums_1.ApplicationStatus.EXECUTED },
            });
    ...
    async getFarmerDeviationList(dateFrom, dateTo) {
        const where = {};
        if (dateFrom || dateTo) {
            const appWhere = {};
            if (dateFrom)
                appWhere.gte = (0, dayjs_1.default)(dateFrom).startOf('day').toDate();
            if (dateTo)
                appWhere.lt = (0, dayjs_1.default)(dateTo).add(1, 'day').startOf('day').toDate();
    ...
    async getChannelWaterBalance(dateStr) {
    ...
    async getFarmerUsageSummary(farmerId, dateFrom, dateTo) {
        const farmer = await this.prisma.farmer.findUnique({ where: { id: farmerId } });
        if (!farmer)
            throw new common_1.NotFoundException('用水户不存在');
        const appWhere = { farmerId };
        if (dateFrom || dateTo) {
            appWhere.targetDate = {};
            if (dateFrom)
                appWhere.targetDate.gte = (0, dayjs_1.default)(dateFrom).startOf('day').toDate();
            if (dateTo)
                appWhere.targetDate.lt = (0, dayjs_1.default)(dateTo).add(1, 'day').startOf('day').toDate();
        }
        ...
        const list = apps.map((a) => {
            ...
            return {
                id: a.id,
                targetDate: (0, dayjs_1.default)(a.targetDate).format('YYYY-MM-DD'),
                status: a.status,
                ...
                evaluation: usage
                    ? usage.isOveruse
                        ...
                        : usage.isWaste
                            ? '浪费'
                            : '正常'
                    : usage === null && a.status === enums_1.ApplicationStatus.SCHEDULED
                        ? '待执行'
                        : '未执行',
        ...
        return {
            ...
            summary: {
                ...
                scheduledCount: apps.filter((a) => a.status === enums_1.ApplicationStatus.SCHEDULED).length,
                executedCount: apps.filter((a) => a.status === enums_1.ApplicationStatus.EXECUTED).length,
                failedCount: apps.filter((a) => a.status === enums_1.ApplicationStatus.FAILED).length,
                cancelledCount: apps.filter((a) => a.status === enums_1.ApplicationStatus.CANCELLED_QUOTA).length,
