...
import { ApplicationStatus, QuotaQuarter, WaterBillStatus, PaymentMethod } from '../common/enums';
import {
...
export class WaterBillingService {
  ...
  async createScheme(dto: CreateWaterPriceSchemeDto) {
    const existing = await this.prisma.waterPriceScheme.findUnique({ where: { code: dto.code } });
    if (existing) throw new BadRequestException('水价方案编码已存在');
  ...
  async updateScheme(id: string, dto: UpdateWaterPriceSchemeDto) {
    const scheme = await this.prisma.waterPriceScheme.findUnique({ where: { id } });
    if (!scheme) throw new NotFoundException('水价方案不存在');
  ...
  async deleteScheme(id: string) {
    ...
    if (hasBindings > 0) throw new BadRequestException('该方案已绑定渠道，无法删除');

    const hasBills = await this.prisma.waterBill.count({ where: { schemeId: id } });
    if (hasBills > 0) throw new BadRequestException('该方案已有账单记录，无法删除');

    return this.prisma.waterPriceScheme.delete({ where: { id } });
  }

  async bindChannelPriceScheme(dto: BindChannelPriceSchemeDto) {
  ...
  async unbindChannelPriceScheme(channelId: string) {
    const existing = await this.prisma.channelPriceBinding.findUnique({ where: { channelId } });
    if (!existing) throw new NotFoundException('该渠道未绑定水价方案');
    return this.prisma.channelPriceBinding.delete({ where: { id: existing.id } });
  }

  async findApplicableSchemeForFarmer(farmerId: string): Promise<{ scheme: any; channel: any } | null> {
    ...
    let currentChannel: any = farmer.channel;
    ...
    while (currentChannel && !visitedChannelIds.has(currentChannel.id)) {
      ...
      if (!currentChannel.parentId) break;
  ...
  async getMonthQuotaVolume(farmerId: string, year: number, month: number): Promise<number> {
  ...
  async calculateMonthWaterUsage(farmerId: string, year: number, month: number) {
    ...
    let totalVolume = 0;
    const details: { applicationId: string; actualVolume: number; estimated: boolean; plannedVolume: number }[] = [];
  ...
  async generateMonthlyBills(dto: GenerateBillsDto) {
    ...
    if (existingBills.length > 0) {
      throw new BadRequestException(`${year}年${month}月账单已生成，如需重新生成请先删除`);
    }
    ...
    for (const farmer of farmers) {
      ...
      const dueDate = dayjs().add(OVERDUE_DAYS, 'day').toDate();
  ...
  async ensureFarmerAccount(farmerId: string) {
  ...
  async getFarmerBills(dto: GetFarmerBillDto) {
  ...
  private formatBillDetail(bill: any, farmer: any) {
    return {
      id: bill.id,
      ...
      tierBreakdown: bill.tierDetails.map((td: any) => ({
        tier: td.tier,
        rangeStart: td.rangeStart,
        rangeEnd: td.rangeEnd,
        unitPrice: td.unitPrice,
      ...
      status: bill.status,
      statusName: WaterBillStatus[bill.status] ? (WaterBillStatus as any)[bill.status] : bill.status,
  ...
  async getChannelBillSummary(dto: ChannelBillSummaryDto) {
    ...
    const farmerChannelMap = new Map<string, string>();
    const farmers = await this.prisma.farmer.findMany({
    ...
    const channelBills = new Map<string, {
      channelId: string;
      channelCode: string;
      channelName: string;
      farmerCount: number;
    ...
    for (const bill of bills) {
      ...
      const farmer = bill.farmer as any;
      ...
      if (bill.status === WaterBillStatus.UNPAID || bill.status === WaterBillStatus.PARTIAL) entry.unpaidCount++;
      if (bill.status === WaterBillStatus.OVERDUE) entry.overdueCount++;
    }
    ...
    const totalSummary = {
      ...
      unpaidCount: bills.filter((b) => b.status === WaterBillStatus.UNPAID || b.status === WaterBillStatus.PARTIAL).length,
      overdueCount: bills.filter((b) => b.status === WaterBillStatus.OVERDUE).length,
      paidCount: bills.filter((b) => b.status === WaterBillStatus.PAID).length,
    };
  ...
  private async getAllChannelDescendants(channelId: string): Promise<string[]> {
    const result: string[] = [];
  ...
  async payWaterBill(dto: PayWaterBillDto) {
    ...
    if (!currentBill) throw new NotFoundException('账单不存在');

    if (currentBill.status === WaterBillStatus.PAID) {
      throw new BadRequestException('账单已缴清');
    }

    const method = dto.method ?? PaymentMethod.FULL;
    ...
    if (method === PaymentMethod.FULL) {
      ...
      if (!dto.amount || dto.amount <= 0) {
        throw new BadRequestException('部分缴费需要指定缴费金额');
      }
      if (dto.amount > currentBill.remainingAmount) {
        throw new BadRequestException(`缴费金额超过待缴金额${currentBill.remainingAmount.toFixed(2)}元`);
      }
    ...
    return {
      ...
      farmer: { id: result.updatedBill.farmerId, name: (result.updatedBill.farmer as any).name },
      ...
      isFullyPaid: result.updatedBill.status === WaterBillStatus.PAID,
      paymentId: result.payment.id,
      accountStatus: account ? { isFrozen: account.isFrozen, freezeReason: account.freezeReason } : null,
    };
  }

  async getFarmerPaymentHistory(dto: GetFarmerPaymentHistoryDto) {
  ...
  async getFarmerDebtStatus(farmerId: string) {
    const farmer = await this.prisma.farmer.findUnique({ where: { id: farmerId } });
    if (!farmer) throw new NotFoundException('用水户不存在');
    ...
    const overdueBills = unpaidBills.filter((b) => b.status === WaterBillStatus.OVERDUE);

    let overdueMonths = 0;
    if (overdueBills.length > 0) {
      const now = dayjs();
      const minBillDate = dayjs(`${overdueBills[0].billingYear}-${String(overdueBills[0].billingMonth).padStart(2, '0')}-01`);
  ...
  async checkFarmerCanApply(farmerId: string): Promise<{ canApply: boolean; reason?: string }> {
  ...
  private async updateSingleBillOverdue(bill: any) {
    if (bill.status === WaterBillStatus.PAID) return;
  ...
  private async updateBillOverdueStatus(farmerId: string) {
    const bills = await this.prisma.waterBill.findMany({
      where: {
        farmerId,
        status: { in: [WaterBillStatus.UNPAID, WaterBillStatus.PARTIAL, WaterBillStatus.OVERDUE] },
  ...
  private async checkAndUpdateFreezeStatus(farmerId: string) {
    const account = await this.ensureFarmerAccount(farmerId);

    const overdueBills = await this.prisma.waterBill.findMany({
      where: { farmerId, status: WaterBillStatus.OVERDUE },
      orderBy: [{ billingYear: 'asc' }, { billingMonth: 'asc' }],
    });
    ...
    if (overdueBills.length > 0) {
      ...
      const firstBillDate = dayjs(`${firstOverdue.billingYear}-${String(firstOverdue.billingMonth).padStart(2, '0')}-01`);
  ...
  private async checkAndUpdateFreezeStatusInTx(tx: any, farmerId: string) {
  ...
  @Cron(CronExpression.EVERY_1ST_DAY_OF_MONTH_AT_MIDNIGHT, { name: 'generate_monthly_bills' })
  ...
  @Cron(CronExpression.EVERY_DAY_AT_2AM, { name: 'check_overdue_bills' })
